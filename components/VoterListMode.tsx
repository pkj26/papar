...
import { Upload, FileSpreadsheet, Play, Loader2, CheckCircle, AlertCircle, Trash2, FileText, Database, PauseCircle, RefreshCw, Zap, TrendingUp, Users, FileDigit } from 'lucide-react';
...
export const VoterListMode: React.FC = () => {
  ...
  const processNextJob = async () => {
    ...
    updateJobStatus(nextId, JobStatus.PROCESSING);
    const job = jobsRef.current.find(j => j.id === nextId);

    if (job) {
        try {
            // Reset stats for new file
            setLiveStats(prev => ({ ...prev, status: `Starting ${job.file.name}...` }));

            const data = await extractVoterData(job.file, (progress) => {
                // Update live stats during processing
                setLiveStats({
                    totalPages: progress.totalPages,
                    processedPages: progress.processedPages,
                    entriesFound: progress.entriesFound,
                    status: progress.status
                });
            });

            if (data.length === 0) {
                updateJobStatus(nextId, JobStatus.ERROR, [], "No entries found. Check file format.");
            } else {
                updateJobStatus(nextId, JobStatus.COMPLETED, data);
            }
        } catch (err: any) {
    ...
    // Done with this file, remove from queue
  ...
  const startProcessing = async () => {
    ...
    const pendingIds = jobs
        .filter(j => j.status === JobStatus.IDLE || j.status === JobStatus.ERROR)
        .map(j => j.id);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "VoterList_Extracted.xlsx");
  };
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              ...
              Voter List Extractor <span className="text-xs bg-purple-100 text-purple-800 px-2 py-0.5 rounded-full flex items-center gap-1 border border-purple-200"><Zap className="w-3 h-3 fill-purple-500 text-purple-500" /> AI Turbo Mode</span>
            </h2>
            <p className="text-slate-500 text-sm mt-1">
              High-Accuracy AI Extraction with live progress tracking.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-lg flex items-center gap-3">
                ...
                <div>
                    <p className="text-xs text-slate-500 font-semibold uppercase">Live Entries Found</p>
                    <p className="text-xl font-bold text-slate-800">
                        {isProcessing ? liveStats.entriesFound : 0}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-lg flex items-center gap-3">
                ...
                <div>
                    <p className="text-xs text-slate-500 font-semibold uppercase">Total Database</p>
                ...
                jobs.map((job, idx) => (
                    <div key={job.id} className={`grid grid-cols-12 p-3 text-sm items-center border-b border-slate-100 transition-colors ${job.status === JobStatus.PROCESSING ? 'bg-indigo-50/30' : 'hover:bg-slate-50'}`}>
                        ...
                        <div className="col-span-3">
                            {job.status === JobStatus.IDLE && <span className="text-slate-400 text-xs bg-slate-100 px-2 py-1 rounded">Pending</span>}
                            ...
                            {job.status === JobStatus.COMPLETED && <span className="text-green-600 text-xs bg-green-50 px-2 py-1 rounded flex items-center w-fit gap-1 border border-green-100"><CheckCircle className="w-3 h-3"/> Done ({job.extractedData.length})</span>}
                            {job.status === JobStatus.ERROR && <span className="text-red-600 text-xs bg-red-50 px-2 py-1 rounded flex items-center w-fit gap-1 border border-red-100" title={job.error}><AlertCircle className="w-3 h-3"/> {job.error?.substring(0, 20)}...</span>}
                        </div>
